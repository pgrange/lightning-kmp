...
class Bolt3RevocationKeyDescriptor(
    ...
    private fun revocationPrivKey(secret: PrivateKey, perCommitSecret: PrivateKey): PrivateKey {
        val a =
            PrivateKey(Crypto.sha256(secret.publicKey().value + perCommitSecret.publicKey().value))
        val b =
            PrivateKey(Crypto.sha256(perCommitSecret.publicKey().value + secret.publicKey().value))
